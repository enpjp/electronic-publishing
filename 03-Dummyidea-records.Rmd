...
if( length(my.record.files) > 0) { # Check we have some records to include.
  ...
  for(my.record in my.record.files) {
    ...
    if(my.record.rds.exists) {
    is.this.my.order <- my.record.data$datumValue[my.record.order]   == "Dummyidea"
    }else{
  ...
  } else if (my.record.data$datumValue[my.record.order]   == "Dummyidea")  {
...
if (fs::file_exists(path.to.data.ss)) {
  my.data.ss <- readRDS(path.to.data.ss)
    my.data.ss.rows.to.keep <- which(my.data.ss$order == "Dummyidea")
...
my.template <- readr::read_file(path.to.readme.template) %>% as.character()
  ...
  rendered.path <- fs::path(path.to.all.data,"_README-Dummyidea", ext = "md")
...
# Remove status chapter as we just keep the md readme.  
  #   rendered.path <- fs::path(absolutePath,"04-status-Dummyidea", ext = "Rmd")
